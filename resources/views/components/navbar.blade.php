    ...
    <div class="flex items-center justify-between h-16 px-4 lg:px-6">
        ...
        <div class="flex items-center space-x-4">
            ...
            <div class="hidden sm:block">
                <nav class="flex items-center space-x-2 text-sm" aria-label="Breadcrumb">
                    <a href="{{ route('dashboard') }}" class="text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 transition-colors">
        ...
        <div class="hidden md:flex flex-1 max-w-md mx-6">
            <div class="relative w-full">
                <input 
                    type="text" 
                    placeholder="Search courses, content, or users..."
                    ...
                    <div 
                        ...
                        <div class="max-h-64 overflow-y-auto">
                            @if ($user?->unreadNotifications()?->count() > 0)
                                @foreach ($user->unreadNotifications()->take(5)->get() as $notification)
                                    <div class="px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-700 border-b border-gray-100 dark:border-gray-600 last:border-0">
                        ...
                        @if ($user?->unreadNotifications()?->count() > 0)
                            <div class="px-4 py-2 border-t border-gray-200 dark:border-gray-700">
                                <a href="{{ route('notifications') }}" class="text-sm text-blue-600 dark:text-blue-400 hover:underline">
                ...
                <div class="relative" x-data="{ open: false }">
                    ...
                    <div 
                        ...
                        @if($user && ($user->hasRole('content_editor') || $user->hasRole('academy_admin') || $user->hasRole('super_admin')))
                            <a href="{{ route('admin.blog.posts.create') }}" class="flex items-center px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                <i class="fas fa-pen w-4 mr-3"></i>
                                New Blog Post
                            </a>
                        @endif
                        <a href="{{ route('feedback') }}" class="flex items-center px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
            ...
            <div class="relative" x-data="{ open: false }">
                <button 
                    ...
                    <div class="hidden sm:block text-left">
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400">{{ ucfirst($user?->getRoleNames()->first() ?? 'User') }}</p>
                    </div>
                ...
                <div 
                    ...
                    @if ($user)
                        ...
                        <a href="{{ route('profile.view') }}" class="flex items-center px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                            <i class="fas fa-user w-4 mr-3"></i>
                            View Profile
                        </a>
                        
                        <a href="{{ route('settings') }}" class="flex items-center px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                    ...
                    @else
                        <a href="{{ route('login') }}" class="flex items-center px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
