    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
        <nav class="flex space-x-4">
            <button wire:click="showBrowse" 
                    class="{{ $currentView === 'browse' ? 'bg-purple-100 text-purple-700 border-purple-200' : 'text-gray-600 hover:text-gray-900' }} px-4 py-2 rounded-lg border transition-colors flex items-center">
                <i class="fas fa-search mr-2"></i>
                Browse Items
            </button>
            
            <button wire:click="showCategories" 
                    class="{{ $currentView === 'categories' ? 'bg-purple-100 text-purple-700 border-purple-200' : 'text-gray-600 hover:text-gray-900' }} px-4 py-2 rounded-lg border transition-colors flex items-center">
            ...
            @if($currentView === 'product-details')
                <button wire:click="backToBrowse" 
                        class="text-gray-600 hover:text-gray-900 px-4 py-2 rounded-lg border transition-colors flex items-center">
                    <i class="fas fa-arrow-left mr-2"></i>
                    Back to Browse
                </button>
                <div class="px-4 py-2 bg-blue-50 text-blue-700 rounded-lg border border-blue-200 flex items-center">
            ...
            @if($search || $type || $category || $minPrice || $maxPrice)
                <div class="mt-4 pt-4 border-t border-gray-200">
                    <div class="flex items-center justify-between">
                        <div class="flex flex-wrap gap-2">
                            @if($search)
                                <span class="inline-flex items-center px-3 py-1 bg-purple-100 text-purple-800 text-sm rounded-full">
                                    Search: "{{ $search }}"
                                    <button wire:click="$set('search', '')" class="ml-2 hover:text-purple-600">
                            ...
                            @if($type)
                                <span class="inline-flex items-center px-3 py-1 bg-blue-100 text-blue-800 text-sm rounded-full">
                                    ...
                                    <button wire:click="$set('type', '')" class="ml-2 hover:text-blue-600">
                                        <i class="fas fa-times text-xs"></i>
                                    </button>
                                </span>
                            @endif
                        ...
                        <button wire:click="clearFilters" class="text-gray-500 hover:text-gray-700 text-sm">
                            Clear All
                        </button>
                    </div>
                </div>
                        ...
                        <div class="lg:col-span-2 space-y-6">
                            ...
                            <div>
                                <div class="flex items-center justify-between mb-2">
                                    <span class="px-3 py-1 bg-{{ $selectedItem->type === 'course' ? 'blue' : ($selectedItem->type === 'resource' ? 'green' : 'orange') }}-100 text-{{ $selectedItem->type === 'course' ? 'blue' : ($selectedItem->type === 'resource' ? 'green' : 'orange') }}-800 text-sm font-medium rounded-full">
                                ...
                                <h1 class="text-2xl font-bold text-gray-900 mb-2">{{ $selectedItem->title }}</h1>
                                <p class="text-gray-600 text-lg">{{ $selectedItem->short_description }}</p>
                            </div>
                                    ...
                                    <div>
                                        @if($selectedItem->hasDiscount())
                                            <div class="flex items-center space-x-3">
                                                <span class="text-3xl font-bold text-purple-600">{{ $selectedItem->getFormattedPrice() }}</span>
                                                <span class="text-xl text-gray-500 line-through">{{ $selectedItem->getFormattedOriginalPrice() }}</span>
                            ...
                            @if($selectedItem->getCategoryNames() || $selectedItem->tags)
                                <div>
                                    <h3 class="text-lg font-medium text-gray-900 mb-3">Categories & Tags</h3>
                                    <div class="flex flex-wrap gap-2">
            ...
            @else
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                    <div class="text-center py-12">
                        ...
                        <p class="text-gray-500 mb-6">The requested item could not be found or is no longer available.</p>
                        <button wire:click="backToBrowse" 
                                class="inline-flex items-center px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors">
                            <i class="fas fa-arrow-left mr-2"></i>
                            Back to Browse
        ...
        @else
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                <div class="text-center py-12">
                    ...
                    <p class="text-gray-500 mb-6">Choose an item from the marketplace to view its details.</p>
                    <button wire:click="backToBrowse" 
                            class="inline-flex items-center px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors">
                        <i class="fas fa-search mr-2"></i>
                        Browse Items
