        ...
        <div class="flex items-center justify-between">
            <nav class="flex space-x-4">
                <button wire:click="showCart" 
                        class="{{ $currentView === 'cart' ? 'bg-purple-100 text-purple-700 border-purple-200' : 'text-gray-600 hover:text-gray-900' }} px-4 py-2 rounded-lg border transition-colors flex items-center">
                ...
                <button wire:click="showPurchases" 
                        class="{{ $currentView === 'purchases' ? 'bg-blue-100 text-blue-700 border-blue-200' : 'text-gray-600 hover:text-gray-900' }} px-4 py-2 rounded-lg border transition-colors flex items-center">
            ...
            @if($currentView === 'cart' && count($cartItems ?? []) > 0)
                <div class="flex space-x-2">
                    <button wire:click="clearCart" 
                            ...
                            class="text-red-600 hover:text-red-700 text-sm px-3 py-1 border border-red-200 rounded hover:bg-red-50 transition-colors">
                                ...
                                <div class="p-6">
                                    <div class="flex items-start space-x-4">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            ...
                                            <p class="text-sm text-gray-600 mt-1 line-clamp-2">{{ $cartItem['item']->short_description }}</p>
                                            <p class="text-sm text-gray-500 mt-1">by {{ $cartItem['item']->vendor->name }}</p>
                                            
                                            <!-- Type and category badges -->
                                            <div class="flex items-center space-x-2 mt-2">
                                                <span class="px-2 py-1 bg-{{ $cartItem['item']->type === 'course' ? 'blue' : ($cartItem['item']->type === 'resource' ? 'green' : 'orange') }}-100 text-{{ $cartItem['item']->type === 'course' ? 'blue' : ($cartItem['item']->type === 'resource' ? 'green' : 'orange') }}-800 text-xs font-medium rounded-full">
                                                ...
                                                <div class="flex items-center space-x-4">
                                                    ...
                                                    <div class="flex items-center space-x-2">
                                                        ...
                                                        <div class="flex items-center border border-gray-300 rounded">
                                                            ...
                                                            <span class="px-3 py-1 text-sm">{{ $cartItem['quantity'] }}</span>
                                                            <button wire:click="updateCartQuantity({{ $cartItem['item']->id }}, {{ $cartItem['quantity'] + 1 }})"
                                                                    class="px-2 py-1 text-gray-500 hover:text-gray-700">
                                                    ...
                                                    <div>
                                                        @if($cartItem['item']->hasDiscount())
                                                            <div class="flex items-center space-x-2">
                                                                ...
                                                                <span class="text-sm text-gray-500 line-through">₦{{ number_format($cartItem['item']->price, 2) }}</span>
                                                                <span class="text-xs bg-red-100 text-red-800 px-1 py-0.5 rounded">
                                                        ...
                                                        @if($cartItem['quantity'] > 1)
                                                            <p class="text-sm text-gray-500">₦{{ number_format($cartItem['price'] * $cartItem['quantity'], 2) }} total</p>
                                                        @endif
                                                    </div>
                                                </div>
                    ...
                    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6 sticky top-6">
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="border-t border-gray-200 pt-3">
                                <div class="flex justify-between">
                                    ...
                                    <span class="text-lg font-bold text-purple-600">₦{{ number_format($cartTotal, 2) }}</span>
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="space-y-3">
                                ...
                                <label class="flex items-center p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <p class="text-sm text-gray-500">Pay with debit/credit card via Paystack</p>
                                    </div>
                                </label>
                            </div>
                        </div>
                        ...
                        <div class="mt-6">
                            ...
                            <textarea wire:model="customerNotes" 
                                      ...
                                      class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-purple-500 focus:border-purple-500 text-sm"></textarea>
                        </div>
                        
                        <!-- Checkout Button -->
                        <button wire:click="checkout"
                                @if($paymentMethod === 'wallet' && $walletBalance < $cartTotal) disabled @endif
                                class="w-full mt-6 px-4 py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors disabled:bg-gray-300 disabled:cursor-not-allowed flex items-center justify-center">
                        ...
                        @if($paymentMethod === 'wallet' && $walletBalance < $cartTotal)
                            <div class="mt-4 p-3 bg-red-50 border border-red-200 rounded-lg">
                                <p class="text-sm text-red-700 text-center flex items-center justify-center">
                                    <i class="fas fa-info-circle mr-2"></i>
                                    Please fund your wallet to complete this purchase
                                </p>
                                <a href="{{ route('wallet.index') }}" 
                                   class="block mt-2 text-center text-sm text-red-600 hover:text-red-700 underline">
                                    Fund Wallet
                                </a>
                            </div>
                        @endif
        ...
        @else
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200">
                <div class="text-center py-16">
                    ...
                    <p class="text-gray-500 mb-8">Browse our marketplace to find great courses, resources, and services.</p>
                    <div class="space-y-3">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div class="flex flex-col md:flex-row md:items-center justify-between space-y-4 md:space-y-0">
                <div>
                    ...
                    <p class="text-gray-600">Track your orders and access purchased items</p>
                </div>
                                        ...
                                        <div class="flex items-start justify-between">
                                            <div class="flex-1">
                                                ...
                                                @if($order->item->short_description)
                                                    <p class="text-sm text-gray-600 mt-2 line-clamp-2">{{ $order->item->short_description }}</p>
                                                @endif
                                            </div>
                                            ...
                                            <div class="text-right ml-4">
                                                ...
                                                <div class="flex flex-col space-y-1 mt-1">
                                                    <span class="inline-flex items-center px-2 py-1 bg-{{ $order->status_color }}-100 text-{{ $order->status_color }}-800 text-xs font-medium rounded-full">
                                                        <i class="fas fa-{{ $order->status === 'completed' ? 'check' : ($order->status === 'pending' ? 'clock' : 'times') }} mr-1"></i>
                                                        {{ ucfirst($order->status) }}
                                                    </span>
                                                    <span class="inline-flex items-center px-2 py-1 bg-{{ $order->payment_status_color }}-100 text-{{ $order->payment_status_color }}-800 text-xs font-medium rounded-full">
                                        ...
                                        @if($order->is_delivered && $order->delivered_at)
                                            <div class="mt-3 p-3 bg-green-50 border border-green-200 rounded-lg">
                                                ...
                                                @if($order->delivery_details && isset($order->delivery_details['notes']))
                                                    <p class="text-sm text-green-700 mt-1">{{ $order->delivery_details['notes'] }}</p>
                                                @endif
                                            </div>
                                        @endif
                                    </div>
