    ...
    <div class="p-4 max-h-96 overflow-y-auto">
        <div id="sectionsList" class="space-y-2">
            @forelse($filteredSections as $section)
                <div class="section-item {{ $activeLessonId && $course->sections->flatMap->lessons->where('id', $activeLessonId)->first()?->course_section_id === $section->id ? 'bg-blue-600 bg-opacity-20 border-blue-500' : 'bg-gray-700 hover:bg-gray-600 border-gray-600' }} rounded-lg border cursor-pointer transition-all duration-200"
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center space-x-3 flex-1">
                                ...
                                <div class="flex-1">
                                    ...
                                    @else
                                        <h3 class="font-semibold {{ $activeLessonId && $course->sections->flatMap->lessons->where('id', $activeLessonId)->first()?->course_section_id === $section->id ? 'text-blue-300' : 'text-white' }}">
                                            {{ $section->title }}
                                        </h3>
                                        @if ($section->description)
                                            <p class="text-xs {{ $activeLessonId && $course->sections->flatMap->lessons->where('id', $activeLessonId)->first()?->course_section_id === $section->id ? 'text-blue-200' : 'text-gray-400' }} mt-1">
                            ...
                            <div class="flex items-center space-x-2">
                                @if ($activeLessonId && $course->sections->flatMap->lessons->where('id', $activeLessonId)->first()?->course_section_id === $section->id)
                                ...
                                @foreach ($section->lessons as $lesson)
                                    <div class="flex items-center justify-between p-2 rounded {{ $activeLessonId && $activeLessonId === $lesson->id ? 'bg-blue-500 bg-opacity-30' : 'hover:bg-gray-600' }} group"
                                        ...
                                        <div class="flex items-center space-x-2 flex-1">
                                            ...
                                            <i class="fas fa-grip-vertical text-gray-500 text-xs cursor-move opacity-0 group-hover:opacity-100 transition-opacity"></i>
                                            <i class="fas fa-{{ $lesson->content_type === 'video' ? 'video' : ($lesson->content_type === 'file' ? 'file' : 'file-text') }} text-xs text-gray-400"></i>
                                            <div class="flex-1">
                                                ...
                                                @else
                                                    <span class="text-sm {{ $activeLessonId && $activeLessonId === $lesson->id ? 'text-white font-medium' : 'text-gray-300' }}">
                                                        {{ $lesson->title }}
                                                    </span>
