        ...
        @foreach($courses as $course)
            ...
            <div class="bg-gray-700 rounded-lg border border-gray-600 hover:border-blue-500 transition-colors overflow-hidden">
                ...
                <div class="flex items-center gap-3">
                    @if($lesson->isAvailableOffline(auth()->user()))
                        <span class="px-2 py-1 bg-green-900 text-green-400 rounded-full text-xs">
                            <i class="fas fa-check-circle mr-1"></i> Available Offline
                        </span>
                    @else
                        <button 
                            wire:click="downloadForOffline('{{ $lesson->id }}')"
                            class="px-2 py-1 bg-gray-700 hover:bg-gray-600 text-gray-300 rounded-full text-xs flex items-center"
                        >
