<div class="space-y-6">
    ...
    @if (session()->has('success'))
        <div
            class="bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300 border border-green-200 dark:border-green-700 p-4 rounded-lg animate__animated animate__fadeIn transition-colors duration-300">
            {{ session('success') }}
        </div>
    @endif
    ...
    <div class="flex items-center justify-between">
        <div>
            <h3 class="text-lg font-medium text-gray-800 dark:text-white transition-colors duration-300">Discussion
                Topics</h3>
            <p class="text-gray-600 dark:text-gray-400 text-sm transition-colors duration-300">Total Points:
                {{ array_sum(array_column($topics, 'points')) }}</p>
        </div>
    ...
    @if ($showCreateForm)
        <div
            class="bg-gray-50 dark:bg-gray-700 rounded-lg p-6 border border-gray-200 dark:border-gray-600 transition-colors duration-300">
            ...
            <form wire:submit.prevent="{{ $editingTopic ? 'updateTopic' : 'createTopic' }}" class="space-y-6">
                ...
                <div>
                    <label
                        class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3 transition-colors duration-300">Topic
                        Type</label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label
                            ...
                            Topic Title <span class="text-red-600 dark:text-red-400">*</span>
                        </label>
                    ...
                    <div>
                        <label
                            class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2 transition-colors duration-300">Points</label>
                ...
                <!-- Discussion Settings -->
                <div
                    class="bg-gray-100 dark:bg-gray-600 rounded-lg p-4 border border-gray-200 dark:border-gray-500 transition-colors duration-300">
                    <h5 class="text-gray-800 dark:text-white font-medium mb-3 transition-colors duration-300">Discussion
                        Requirements</h5>

                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                        <div>
                            <label
                                class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2 transition-colors duration-300">Min
                                Responses</label>
                        ...
                        <div>
                            <label
                                class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2 transition-colors duration-300">Time
                                Limit (minutes)</label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div class="flex items-center">
                            ...
                            <label for="allowPeerReview"
                                class="ml-2 block text-sm text-gray-700 dark:text-gray-300 transition-colors duration-300">
                                Allow Peer Review
                            </label>
                        </div>
                        <div class="flex items-center">
                ...
                <div
                    ...
                    <div class="flex items-center justify-between mb-3">
                        <h5 class="text-gray-800 dark:text-white font-medium transition-colors duration-300">Discussion
                            Prompts</h5>
                ...
                <div class="bg-gray-600 rounded-lg p-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-3 mb-3 p-3 bg-gray-200 dark:bg-gray-700 rounded-lg border border-gray-300 dark:border-gray-600 transition-colors duration-300">
                        <div>
...
<!-- Topics List -->
<div class="space-y-4">
    <h4 class="text-gray-800 dark:text-white font-medium transition-colors duration-300">Discussion Topics ({{ count($topics) }})</h4>
    ...
    @if (count($topics) > 0)
                                ...
                                <div class="flex-1">
                                    <div class="flex items-center gap-2 mb-2">
                                        <span class="px-2 py-1 text-xs rounded-full border transition-colors duration-300
                                            {{ $topicType === 'discussion' ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300 border-blue-200 dark:border-blue-700' : 
                                               ($topicType === 'debate' ? 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300 border-red-200 dark:border-red-700' : 
                                                ($topicType === 'peer_review' ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300 border-green-200 dark:border-green-700' : 
                                                 ($topicType === 'reflection' ? 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-300 border-yellow-200 dark:border-yellow-700' : 
                                        ...
                                        <span class="text-sm text-green-600 dark:text-green-400 font-medium">{{ $topic['points'] }} pts</span>
                                        @if ($topic['is_required'])
                                            <span class="text-xs text-red-600 dark:text-red-400">Required</span>
                                        @endif
                                        @if ($topic['time_limit'])
                                            <span class="text-sm text-blue-600 dark:text-blue-400">{{ $topic['time_limit'] }}min</span>
                                        @endif
                                    </div>
                                    
                                    <p class="text-gray-800 dark:text-white font-medium mb-2 transition-colors duration-300">{{ $topic['question_text'] }}</p>
                                    
                                    @if ($topic['explanation'])
                                        <p class="text-gray-600 dark:text-gray-400 text-sm mb-2 transition-colors duration-300">{{ $topic['explanation'] }}</p>
                                    @endif

                                    <!-- Topic Requirements -->
                                    @if (!empty($topicData['min_responses']) || !empty($topicData['min_response_length']))
                                        <div class="text-sm text-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-900/20 px-2 py-1 rounded mb-2 border border-blue-200 dark:border-blue-700 transition-colors duration-300">
                                    ...
                                    @if (!empty($topicData['discussion_prompts']))
                                        <div class="text-sm text-purple-700 dark:text-purple-300 bg-purple-50 dark:bg-purple-900/20 px-2 py-1 rounded border border-purple-200 dark:border-purple-700 transition-colors duration-300">
        ...
        <div class="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 text-center border border-gray-200 dark:border-gray-600 transition-colors duration-300">
                <div class="text-xl font-bold text-blue-600 dark:text-blue-400">{{ $typeStats['discussion'] ?? 0 }}</div>
                <div class="text-xs text-gray-600 dark:text-gray-400">Discussions</div>
            </div>

            <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 text-center border border-gray-200 dark:border-gray-600 transition-colors duration-300">
                <div class="text-xl font-bold text-red-600 dark:text-red-400">{{ $typeStats['debate'] ?? 0 }}</div>
                <div class="text-xs text-gray-600 dark:text-gray-400">Debates</div>
            </div>

            <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 text-center border border-gray-200 dark:border-gray-600 transition-colors duration-300">
                <div class="text-xl font-bold text-green-600 dark:text-green-400">{{ $typeStats['peer_review'] ?? 0 }}</div>
                <div class="text-xs text-gray-600 dark:text-gray-400">Peer Reviews</div>
            </div>
    ...
    @else
        ...
        <div class="text-center py-8">
            <div class="w-16 h-16 bg-gray-200 dark:bg-gray-700 rounded-full flex items-center justify-center mx-auto mb-4 border border-gray-300 dark:border-gray-600 transition-colors duration-300">
                <i class="fas fa-comments text-2xl text-gray-500 dark:text-gray-400"></i>
            </div>
            <h4 class="text-lg font-medium text-gray-800 dark:text-white mb-2 transition-colors duration-300">No Discussion Topics Yet</h4>
            <p class="text-gray-600 dark:text-gray-400 mb-4 transition-colors duration-300">Create discussion topics to enable Q&A based evaluation.</p>
            <button wire:click="toggleCreateForm"
                    class="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg transition-colors">
                <i class="fas fa-plus mr-2"></i>
