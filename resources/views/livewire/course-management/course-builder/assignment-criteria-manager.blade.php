<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h3 class="text-lg font-medium text-gray-800 dark:text-white transition-colors duration-300">Assignment Questions</h3>
            <p class="text-gray-600 dark:text-gray-400 text-sm transition-colors duration-300">Total Points: {{ array_sum(array_column($questions, 'points')) }}</p>
        </div>
    ...
    @if ($showCreateForm)
        <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-6 border border-gray-200 dark:border-gray-600 transition-colors duration-300">
            ...
            <form wire:submit.prevent="{{ $editingQuestion ? 'updateQuestion' : 'createQuestion' }}" class="space-y-6">
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3 transition-colors duration-300">Assignment Type</label>
                            ...
                            <button type="button" wire:click="selectQuestionType('{{ $type }}')"
                                class="p-3 rounded-lg text-center transition-colors border-2 text-sm
                                        {{ $questionType === $type
                                            ...
                                            : 'bg-white dark:bg-gray-600 border-gray-300 dark:border-gray-500 text-gray-800 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-500 hover:border-gray-400 dark:hover:border-gray-400' }} duration-300">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2 transition-colors duration-300">Word Limit</label>
                ...
                <div class="bg-gray-100 dark:bg-gray-600 rounded-lg p-4 border border-gray-200 dark:border-gray-500 transition-colors duration-300">
                    <div class="flex items-center justify-between mb-3">
                        <h5 class="text-gray-800 dark:text-white font-medium transition-colors duration-300">Grading Rubric</h5>
                    ...
                    @foreach ($rubricCriteria as $index => $criteria)
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-3 mb-3 p-3 bg-gray-200 dark:bg-gray-700 rounded-lg border border-gray-300 dark:border-gray-600 transition-colors duration-300">
                            <div>
                                <label class="block text-xs text-gray-600 dark:text-gray-400 mb-1 transition-colors duration-300">Criteria Name</label>
                                <input type="text" wire:model="rubricCriteria.{{ $index }}.name"
                                    class="w-full px-3 py-2 bg-white dark:bg-gray-600 border border-gray-300 dark:border-gray-500 rounded-lg text-gray-900 dark:text-white text-sm transition-colors duration-300"
                                    placeholder="e.g., Content Knowledge">
                            </div>
                            <div>
                                <label class="block text-xs text-gray-600 dark:text-gray-400 mb-1 transition-colors duration-300">Weight (%)</label>
                            ...
                            <div>
                                <label class="block text-xs text-gray-600 dark:text-gray-400 mb-1 transition-colors duration-300">Description</label>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2 transition-colors duration-300">Sample Answer (Optional)</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2 transition-colors duration-300">Grading Notes (Optional)</label>
                ...
                <div class="bg-gray-100 dark:bg-gray-600 rounded-lg p-4 border border-gray-200 dark:border-gray-500 transition-colors duration-300">
                    <div class="flex items-center justify-between mb-3">
                        <h5 class="text-gray-800 dark:text-white font-medium transition-colors duration-300">File Upload Settings</h5>
                        <label class="flex items-center">
                            <input type="checkbox" wire:model="allowFileUpload"
                                class="h-4 w-4 text-yellow-600 focus:ring-yellow-500 border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700">
                            <span class="ml-2 text-sm text-gray-700 dark:text-gray-300 transition-colors duration-300">Allow file uploads</span>
                        </label>
                    </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                @if (count($fileTypes) > 0)
                                    @foreach ($fileTypes as $index => $fileType)
                                        <div class="flex gap-2 mb-2">
                                            <input type="text" wire:model="fileTypes.{{ $index }}"
                                                class="flex-1 px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white text-sm transition-colors duration-300"
                                    ...
                                    @endforeach
                                @else
                                    <p class="text-sm text-gray-600 dark:text-gray-400 transition-colors duration-300">No file types specified - all types allowed</p>
                                @endif
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" wire:model="maxFileSize" min="1" max="100"
                                    class="w-32 px-3 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg text-gray-900 dark:text-white transition-colors duration-300">
                                @error('maxFileSize')
                                    <span class="text-red-600 dark:text-red-400 text-sm mt-1">{{ $message }}</span>
                                @enderror
                            </div>
    ...
    <!-- Questions List -->
    <div class="space-y-4">
        <h4 class="text-gray-800 dark:text-white font-medium transition-colors duration-300">Assignment Questions ({{ count($questions) }})</h4>
        ...
        @if (count($questions) > 0)
                            ...
                            <div class="flex-1">
                                <div class="flex items-start justify-between mb-2">
                                    <div class="flex-1">
                                                    ...
                                                    : ($assignmentType === 'short_answer'
                                                        ...
                                                        : ($assignmentType === 'analysis'
                                                            ...
                                                            : ($assignmentType === 'reflection'
                                                                ...
                                                                : 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300 border border-red-200 dark:border-red-700'))) }} transition-colors duration-300">
                                        ...
                                        <p class="text-gray-800 dark:text-white font-medium mb-2 transition-colors duration-300">{{ $question['question_text'] }}</p>

                                        @if ($question['explanation'])
                                            <div class="text-sm text-gray-600 dark:text-gray-400 bg-gray-100 dark:bg-gray-800 px-3 py-2 rounded mb-2 border border-gray-200 dark:border-gray-700 transition-colors duration-300">
            ...
            <div class="mt-6 grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 text-center border border-gray-200 dark:border-gray-600 transition-colors duration-300">
                    ...
                    <div class="text-xs text-gray-600 dark:text-gray-400">Essays</div>
                </div>

                <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 text-center border border-gray-200 dark:border-gray-600 transition-colors duration-300">
                    <div class="text-xl font-bold text-blue-600 dark:text-blue-400">
                        {{ ($typeStats['analysis'] ?? 0) + ($typeStats['research'] ?? 0) }}</div>
                    <div class="text-xs text-gray-600 dark:text-gray-400">Research/Analysis</div>
                </div>

                <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-3 text-center border border-gray-200 dark:border-gray-600 transition-colors duration-300">
                    <div class="text-xl font-bold text-green-600 dark:text-green-400">{{ $timedQuestions }}</div>
                    <div class="text-xs text-gray-600 dark:text-gray-400">Timed Questions</div>
                </div>
            </div>
        @else
            <!-- Empty State -->
            <div class="text-center py-8">
                ...
                <h4 class="text-lg font-medium text-gray-800 dark:text-white mb-2 transition-colors duration-300">No Assignment Questions Yet</h4>
                <p class="text-gray-600 dark:text-gray-400 mb-4 transition-colors duration-300">Create assignment questions to evaluate student understanding through
                    written work.</p>
                <button wire:click="toggleCreateForm"
                    class="px-4 py-2 bg-yellow-600 hover:bg-yellow-700 text-white rounded-lg transition-colors">
                    <i class="fas fa-plus mr-2"></i>
