    ...
    @else
        ...
        <div class="mb-6 flex space-x-4" x-data="{ selectedCourses: @entangle('selectedCourses') }">
            @can('publish-courses')
                <button wire:click="bulkPublish"
                    class="bg-blue-600 text-white px-4 py-2 rounded-xl hover:bg-blue-700 transition duration-300 disabled:opacity-50 flex items-center"
                    x-bind:disabled="!selectedCourses.length" aria-label="Publish selected courses">
                    <i class="fas fa-globe mr-2"></i> Publish
                </button>
                <button wire:click="bulkUnpublish"
                    class="bg-gray-600 text-white px-4 py-2 rounded-xl hover:bg-gray-700 transition duration-300 disabled:opacity-50 flex items-center"
                    x-bind:disabled="!selectedCourses.length" aria-label="Unpublish selected courses">
                    <i class="fas fa-eye-slash mr-2"></i> Unpublish
                </button>
            @endcan
            @can('approve-courses')
                <button wire:click="bulkApprove"
                    class="bg-green-600 text-white px-4 py-2 rounded-xl hover:bg-green-700 transition duration-300 disabled:opacity-50 flex items-center"
                    x-bind:disabled="!selectedCourses.length" aria-label="Approve selected courses">
                    <i class="fas fa-check-circle mr-2"></i> Approve
                </button>
            @endcan
            @can('delete-courses')
                <button wire:click="bulkDelete"
                    class="bg-red-600 text-white px-4 py-2 rounded-xl hover:bg-red-700 transition duration-300 disabled:opacity-50 flex items-center"
                    x-bind:disabled="!selectedCourses.length" aria-label="Delete selected courses">
                    <i class="fas fa-trash-alt mr-2"></i> Delete
                </button>
            @endcan
            <button wire:click="exportSelected"
                class="bg-teal-600 text-white px-4 py-2 rounded-xl hover:bg-teal-700 transition duration-300 disabled:opacity-50 flex items-center"
                x-bind:disabled="!selectedCourses.length" aria-label="Export selected courses">
        ...
        @if ($courses->isEmpty())
            <div class="text-center py-12">
                <p class="text-teal-300 text-xl animate__animated animate__pulse">No courses found matching your
                    criteria.</p>
                    <div class="py-4">
                    <a href="{{ route('course-categories') }}" 
                        class="mx-4 bg-green-600 text-white px-6 py-3 rounded-xl text-center hover:bg-blue-700 transition duration-300 shadow-md items-center animate__animated animate__pulse animate__infinite">
                        <i class="fas fa-plus mr-2"></i>
                        Check available Categories
                    </a>
                    <a href="{{ route('create_course') }}" 
                    class="bg-blue-600 text-white px-6 py-3 rounded-xl text-center hover:bg-green-700 transition duration-300 shadow-md items-center animate__animated animate__pulse animate__infinite">
                    <i class="fas fa-plus mr-2"></i>
                    Create Course
                </a>
        ...
        @else
                ...
                <table wire:poll.10s class="min-w-full divide-y divide-teal-700" aria-label="Courses Table">
                    <thead class="bg-indigo-800">
                        <tr>
                            ...
                            <th wire:click="sortBy('title')"
                                class="px-6 py-4 text-left text-xs font-medium text-teal-200 uppercase tracking-wider cursor-pointer"
                                aria-sort="{{ $sortField === 'title' ? $sortDirection : 'none' }}">
                                Title @if ($sortField === 'title')
                                    <i
                                        class="fas fa-sort-{{ $sortDirection === 'asc' ? 'up' : 'down' }} text-teal-300"></i>
                                @endif
                            </th>
                            <th wire:click="sortBy('instructor.name')"
                                class="px-6 py-4 text-left text-xs font-medium text-teal-200 uppercase tracking-wider cursor-pointer hidden md:table-cell"
                                aria-sort="{{ $sortField === 'instructor.name' ? $sortDirection : 'none' }}">
                                Instructor @if ($sortField === 'instructor.name')
                                    <i
                                        class="fas fa-sort-{{ $sortDirection === 'asc' ? 'up' : 'down' }} text-teal-300"></i>
                                @endif
                            </th>
                            <th wire:click="sortBy('category.name')"
                                class="px-6 py-4 text-left text-xs font-medium text-teal-200 uppercase tracking-wider cursor-pointer"
                                aria-sort="{{ $sortField === 'category.name' ? $sortDirection : 'none' }}">
                                Category @if ($sortField === 'category.name')
                                    <i
                                        class="fas fa-sort-{{ $sortDirection === 'asc' ? 'up' : 'down' }} text-teal-300"></i>
                                @endif
                            </th>
                            <th wire:click="sortBy('enrollments_count')"
                                class="px-6 py-4 text-left text-xs font-medium text-teal-200 uppercase tracking-wider cursor-pointer"
                                x-on:mouseover="tooltip = 'Number of enrolled students'" x-on:mouseout="tooltip = ''">
                                Enrollments @if ($sortField === 'enrollments_count')
                                    <i
                                        class="fas fa-sort-{{ $sortDirection === 'asc' ? 'up' : 'down' }} text-teal-300"></i>
                                @endif
                            </th>
                            <th wire:click="sortBy('difficulty_level')"
                                class="px-6 py-4 text-left text-xs font-medium text-teal-200 uppercase tracking-wider cursor-pointer"
                                aria-sort="{{ $sortField === 'difficulty_level' ? $sortDirection : 'none' }}">
                                Difficulty @if ($sortField === 'difficulty_level')
                                    <i
                                        class="fas fa-sort-{{ $sortDirection === 'asc' ? 'up' : 'down' }} text-teal-300"></i>
                                @endif
                            </th>
                            ...
                            <th class="px-6 py-4 text-left text-xs font-medium text-teal-200 uppercase tracking-wider">
                                Status</th>
                            <th
                                class="px-6 py-4 text-right text-xs font-medium text-teal-200 uppercase tracking-wider">
                                Actions</th>
                        </tr>
                    </thead>
                    <tbody class="bg-indigo-950 divide-y divide-teal-800">
                        @foreach ($courses as $course)
                            <tr
                                class="hover:bg-indigo-900 transition duration-200 ease-in-out animate__animated animate__fadeIn">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap hidden md:table-cell text-teal-300">
                                    {{ $course->instructor->name }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-teal-300">{{ $course->category->name }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-teal-300">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                                        ...
                                        x-on:mouseover="tooltip = 'Difficulty: {{ ucfirst($course->difficulty_level) }}'"
                                        x-on:mouseout="tooltip = ''">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @can('publish-courses')
                                        <button wire:click="togglePublished({{ $course->id }})"
                                            ...
                                            x-on:mouseover="tooltip = '{{ $course->is_published ? 'Unpublish' : 'Publish' }} course'"
                                            x-on:mouseout="tooltip = ''">
                                    ...
                                    @can('approve-courses')
                                        ...
                                        @else
                                            <button wire:click="toggleApproved({{ $course->id }})"
                                                ...
                                                x-on:mouseover="tooltip = '{{ $course->status === 'approved' ? 'Unapprove' : 'Approve' }} course'"
                                                x-on:mouseout="tooltip = ''">
                                                {{ ucfirst($course->status) }}
                                            </button>
                                        @endif
            ...
            <div class="mt-8 flex justify-between items-center">
                <a href="{{ route('create_course') }}"
                    class="bg-green-600 text-white px-6 py-3 rounded-xl hover:bg-green-700 transition duration-300 shadow-md flex items-center animate__animated animate__pulse animate__infinite"
                    aria-label="Create a new course">
        ...
        <!-- Tooltip -->
        <div x-show="tooltip" class="fixed bg-indigo-900 text-white text-sm px-3 py-1 rounded-lg shadow-lg"
            x-text="tooltip" style="z-index: 1000;"></div>
    @endif
</div>
