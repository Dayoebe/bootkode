<div class="px-4 py-6">
    <!-- Page Header -->
    <div class="mb-8">
        <h2 class="text-3xl font-bold text-gray-900 mb-2">CBT Examinations</h2>
        <p class="text-gray-600">Select an assessment to begin your computer-based test</p>
    </div>
    ...
    @if($availableAssessments->count() > 0)
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($availableAssessments as $assessment)
                <div class="bg-white border border-gray-200 rounded-lg shadow-sm hover:shadow-md transition-shadow border-l-4 
                    {{ $assessment->user_result && $assessment->user_result['passed'] ? 'border-l-green-500' : 'border-l-blue-500' }}">
                    
                    <div class="p-6">
                        <!-- Assessment Header -->
                        <div class="flex justify-between items-start mb-4">
                            <h3 class="text-lg font-semibold text-gray-900 leading-tight">{{ $assessment->title }}</h3>
                            @if($assessment->user_result)
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                    {{ $assessment->user_result['passed'] ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                        ...
                        @if($assessment->description)
                            <p class="text-gray-600 text-sm mb-4 line-clamp-2">{{ $assessment->description }}</p>
                        @endif
                        ...
                        <div class="grid grid-cols-3 gap-3 mb-4">
                            <div class="text-center">
                                ...
                                <div class="font-semibold text-gray-900">{{ $assessment->questions->count() }}</div>
                            </div>
                            <div class="text-center">
                                <div class="text-xs text-gray-500 mb-1">Duration</div>
                                <div class="font-semibold text-gray-900">{{ $assessment->formatted_duration }}</div>
                            </div>
                            <div class="text-center">
                                <div class="text-xs text-gray-500 mb-1">Pass %</div>
                                <div class="font-semibold text-gray-900">{{ $assessment->pass_percentage }}%</div>
                            </div>
                        </div>
                        ...
                        @if($assessment->user_result)
                            <div class="bg-gray-50 rounded-lg p-3 mb-4">
                                <div class="flex justify-between items-center text-sm">
                                    ...
                                    <span class="font-semibold {{ $assessment->user_result['passed'] ? 'text-green-600' : 'text-red-600' }}">
                                ...
                                @if($assessment->attempts_count > 1)
                                    <div class="flex justify-between items-center text-sm mt-1">
                                        ...
                                        <span class="text-gray-900">{{ $assessment->attempts_count }}</span>
                                    </div>
                                @endif
                            </div>
                        @endif
                        ...
                        <div class="space-y-2">
                            ...
                            @if($assessment->user_result)
                                <button wire:click="viewResults({{ $assessment->id }})"
                                    class="w-full bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2 rounded-lg transition-colors flex items-center justify-center font-medium">
        ...
        @if($completedAssessments > 0)
            <div class="mt-8 bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">Your Progress Summary</h3>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div class="text-center">
                        <div class="text-2xl font-bold text-blue-600">{{ $completedAssessments }}</div>
                        <div class="text-sm text-gray-500">Completed</div>
                    </div>
                    <div class="text-center">
                        <div class="text-2xl font-bold text-green-600">{{ $passedAssessments }}</div>
                        <div class="text-sm text-gray-500">Passed</div>
                    </div>
                    <div class="text-center">
                        <div class="text-2xl font-bold text-yellow-600">{{ $totalAssessments - $completedAssessments }}</div>
    ...
    @else
        ...
        <div class="text-center py-16">
            ...
            <h3 class="text-xl font-semibold text-gray-500 mb-2">No CBT Assessments Available</h3>
            <p class="text-gray-400 mb-6">There are currently no computer-based tests available for you to take.</p>
