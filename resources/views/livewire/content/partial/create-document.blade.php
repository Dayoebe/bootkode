    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-4 md:space-y-0">
            <div>
                ...
                <p class="text-gray-600 mt-1">Step {{ $currentStep }} of {{ $totalSteps }}</p>
            </div>
            
            <div class="flex items-center space-x-2">
                <button wire:click="saveDraft" class="text-gray-700 bg-gray-200 px-4 py-2 rounded-lg hover:bg-gray-300 transition-colors">
                    Save Draft
                </button>
            </div>
        </div>
                ...
                <div class="flex items-center {{ $step < $totalSteps ? 'flex-1' : '' }}">
                    <button 
                        ...
                        class="flex items-center justify-center w-8 h-8 rounded-full text-sm font-medium transition-colors
                            ...
                            {{ $step < $currentStep ? 'cursor-pointer hover:bg-indigo-700' : '' }}
                        "
                    >
                        @if($step < $currentStep)
                    ...
                    @if($step < $totalSteps)
                        <div class="flex-1 mx-2 h-0.5 {{ $step < $currentStep ? 'bg-indigo-600' : 'bg-gray-200' }}"></div>
                    @endif
                </div>
            @endfor
        </div>
        ...
        <div class="mt-2 flex items-center justify-between text-xs text-gray-500">
            ...
            <span class="{{ $currentStep >= 2 ? 'text-indigo-600 font-medium' : '' }}">Content</span>
            <span class="{{ $currentStep >= 3 ? 'text-indigo-600 font-medium' : '' }}">Settings</span>
            <span class="{{ $currentStep >= 4 ? 'text-indigo-600 font-medium' : '' }}">Review</span>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        <form wire:submit.prevent class="space-y-6">
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select wire:model="type" id="type" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500" required>
                        ...
                        <div>
                            ...
                            <select wire:model="category_id" id="category_id" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">
                        ...
                        <div>
                            ...
                            <select wire:model="difficulty_level" id="difficulty_level" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500" required>
                        ...
                        <div>
                            ...
                            <select wire:model="visibility" id="visibility" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">
            ...
            @if($currentStep === 2)
                <!-- Step 2: Content -->
                <div class="space-y-6">
                    <h3 class="text-lg font-semibold text-gray-900 border-b pb-2">Document Content</h3>
                    
                    <div>
                        <label for="excerpt" class="block text-sm font-medium text-gray-700 mb-2">Excerpt/Summary</label>
                        <textarea 
                            wire:model="excerpt" 
                            id="excerpt" 
                            rows="3" 
                        ...
                        ></textarea>
                        <p class="text-sm text-gray-500 mt-1">This will be displayed in search results and document listings.</p>
                    ...
                    <div>
                        ...
                        <div class="flex justify-between items-center mt-2">
                            <p class="text-sm text-gray-500">Markdown formatting is supported.</p>
                            <span class="text-sm text-gray-400">{{ strlen($content ?? '') }} characters</span>
                        </div>
            ...
            @if($currentStep === 3)
                <!-- Step 3: Settings & SEO -->
                <div class="space-y-6">
                    <h3 class="text-lg font-semibold text-gray-900 border-b pb-2">Settings & SEO</h3>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            >
                            <p class="text-sm text-gray-500 mt-1">{{ 60 - strlen($meta_title ?? '') }} characters remaining</p>
                            @error('meta_title') <span class="text-red-500 text-sm mt-1">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <label for="status" class="block text-sm font-medium text-gray-700 mb-2">Status</label>
                            <select wire:model="status" id="status" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">
                        ...
                        <div class="md:col-span-2">
                            ...
                            ></textarea>
                            <p class="text-sm text-gray-500 mt-1">{{ 160 - strlen($meta_description ?? '') }} characters remaining</p>
                        ...
                        <div class="md:col-span-2">
                            <label class="flex items-center space-x-3">
                                ...
                                <span class="text-sm font-medium text-gray-700">Featured Document</span>
                            </label>
                            <p class="text-sm text-gray-500 mt-1 ml-6">Featured documents appear prominently on the site</p>
                        </div>
                    </div>
                </div>
            @endif
            ...
            @if($currentStep === 4)
                ...
                <div class="space-y-6">
                    <h3 class="text-lg font-semibold text-gray-900 border-b pb-2">Review & Publish</h3>
                    ...
                    <div class="bg-gray-50 rounded-lg p-6 space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <p class="text-gray-900">{{ $title ?: 'Not set' }}</p>
                            </div>
                            <div>
                                <label class="text-sm font-medium text-gray-700">Type:</label>
                                <p class="text-gray-900">{{ $types[$type] ?? 'Not set' }}</p>
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <p class="text-gray-900">{{ $difficultyLevels[$difficulty_level] ?? 'Not set' }}</p>
                            </div>
                            <div>
                                <label class="text-sm font-medium text-gray-700">Status:</label>
                                <p class="text-gray-900">{{ $statuses[$status] ?? 'Not set' }}</p>
                            </div>
                            <div>
                                <label class="text-sm font-medium text-gray-700">Visibility:</label>
                                <p class="text-gray-900">{{ $visibilities[$visibility] ?? 'Not set' }}</p>
                            </div>
                        </div>
                        ...
                        @if($excerpt)
                            <div>
                                ...
                                <p class="text-gray-900 text-sm">{{ $excerpt }}</p>
                            </div>
                        @endif
                        ...
                        <div>
                            ...
                            <p class="text-gray-900">{{ strlen($content ?? '') }} characters, ~{{ ceil(strlen($content ?? '') / 5) }} words</p>
                        </div>

                        @if($featured)
                            <div class="flex items-center space-x-2">
                                <i class="fas fa-star text-yellow-500"></i>
                                <span class="text-sm font-medium text-gray-700">This document will be featured</span>
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="bg-blue-50 rounded-lg p-4">
                        <h4 class="text-sm font-medium text-blue-900 mb-2">Publishing Options</h4>
                ...
                <div>
                    @if($currentStep > 1)
                        <button 
                            ...
                            class="px-6 py-2 text-gray-700 bg-gray-200 rounded-lg hover:bg-gray-300 transition-colors flex items-center space-x-2"
                        >
...
document.addEventListener('input', function(e) {
    if (e.target.matches('#meta_title')) {
        ...
        if (counter) {
            ...
            counter.className = remaining < 0 ? 'text-red-500 text-sm mt-1' : 'text-gray-500 text-sm mt-1';
        }
    }
