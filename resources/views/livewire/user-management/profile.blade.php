...
<div class="min-h-screen bg-themed-primary p-4 transition-colors duration-300">
    <div x-data="{ activeTab: @entangle('activeTab') }">
        ...
        <!-- Header Section -->
        <div
            class="relative bg-themed-secondary backdrop-blur-sm rounded-2xl lg:rounded-3xl shadow-lg lg:shadow-2xl border border-themed-primary overflow-hidden mb-6 lg:mb-8 transition-colors duration-300">
            <!-- Background Pattern -->
            <div class="absolute inset-0 bg-[url('data:image/svg+xml,%3Csvg width=" 60" height="60" viewBox="0 0 60 60"
                xmlns="http://www.w3.org/2000/svg" %3E%3Cg fill="none" fill-rule="evenodd" %3E%3Cg fill="%23000000"
                fill-opacity="0.05" %3E%3Ccircle cx="30" cy="30" r="2" /%3E%3C/g%3E%3C/g%3E%3C/svg%3E')] opacity-50">
            </div>
                    ...
                    <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6 flex-1">
                        <div class="relative group">
                            ...
                            @if($user->profile_picture)
                                <img src="{{ $user->profile_picture }}" class="w-32 h-32 rounded-full object-cover"
                                    alt="{{ $user->name }}">
                            @else
                                            <!-- fallback avatar -->
                            ...
                            <!-- Status Indicator -->
                            <div
                                class="absolute bottom-0 right-0 w-5 h-5 sm:w-6 sm:h-6 lg:w-8 lg:h-8 bg-green-500 rounded-full border-4 border-themed-secondary flex items-center justify-center">
                        ...
                        <div class="flex-1 min-w-0">
                            <h1
                                class="text-2xl sm:text-3xl lg:text-4xl font-bold text-themed-primary mb-2 truncate transition-colors duration-300">
                                {{ $user->name }}
                            </h1>
                            ...
                            <div class="flex flex-wrap items-center gap-2 mb-3">
                                <span
                                    class="px-3 py-1 sm:px-4 sm:py-1.5 rounded-full text-xs sm:text-sm font-medium bg-accent-themed-primary text-white border border-accent-themed-primary/30 backdrop-blur-sm">
                                ...
                                @if ($user->email_verified_at)
                                    <span
                                        class="px-2 py-1 sm:px-3 sm:py-1.5 rounded-full text-xs sm:text-sm font-medium bg-green-500/20 border border-green-500/30 text-green-600 backdrop-blur-sm flex items-center">
                                        <i class="fas fa-check-circle mr-1"></i> Verified
                                    </span>
                                @else
                                    <span
                                        class="px-2 py-1 sm:px-3 sm:py-1.5 rounded-full text-xs sm:text-sm font-medium bg-yellow-500/20 border border-yellow-500/30 text-yellow-600 backdrop-blur-sm flex items-center">
                                ...
                                @if ($user->is_active)
                                    <span
                                        class="px-2 py-1 sm:px-3 sm:py-1.5 rounded-full text-xs sm:text-sm font-medium bg-emerald-500/20 border border-emerald-500/30 text-emerald-600 backdrop-blur-sm flex items-center">
                            ...
                            @if ($user->bio)
                                <p
                                    class="text-themed-secondary text-sm lg:text-base leading-relaxed transition-colors duration-300">
                                    {{ $user->bio }}
                                </p>
                            @endif
                        </div>
        ...
        <!-- Tab Navigation -->
        <div
            class="bg-themed-secondary backdrop-blur-sm rounded-2xl border border-themed-primary mb-6 lg:mb-8 overflow-hidden transition-colors duration-300">
        ...
        <!-- Tab Content -->
        <div
            class="bg-themed-secondary backdrop-blur-sm rounded-2xl border border-themed-primary overflow-hidden shadow-lg lg:shadow-xl transition-colors duration-300">
            @if ($isEditing)
                <!-- Edit Mode Content -->
