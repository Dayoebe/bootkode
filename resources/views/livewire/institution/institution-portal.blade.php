    ...
    <div class="w-full mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white shadow-sm border-b border-gray-200">
            <div class="px-4 sm:px-6 lg:px-8">
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center py-4 md:py-6 space-y-4 md:space-y-0">
                    <div class="flex items-center space-x-4">
                        ...
                        <div class="animate__animated animate__fadeInUp">
                            <h1 class="text-2xl md:text-3xl font-bold text-gray-900">Institution Portal</h1>
                            <p class="text-gray-600 mt-1 text-sm md:text-base">Manage institutional partnerships and licensing</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center space-x-4 md:space-x-6 animate__animated animate__fadeInRight overflow-x-auto pb-2 md:pb-0 w-full md:w-auto">
                        <div class="text-center min-w-[80px] md:min-w-0">
                            <div class="text-xl md:text-2xl font-bold text-blue-600">{{ number_format($stats['total_institutions']) }}</div>
                            <div class="text-xs text-gray-500">Institutions</div>
                        </div>
                        <div class="text-center min-w-[80px] md:min-w-0">
                            <div class="text-xl md:text-2xl font-bold text-green-600">{{ number_format($stats['active_institutions']) }}</div>
                            <div class="text-xs text-gray-500">Active</div>
                        </div>
                        <div class="text-center min-w-[80px] md:min-w-0">
                            <div class="text-xl md:text-2xl font-bold text-purple-600">{{ number_format($stats['total_users']) }}</div>
                            <div class="text-xs text-gray-500">Users</div>
                        </div>
                        <div class="text-center min-w-[80px] md:min-w-0">
                            <div class="text-xl md:text-2xl font-bold text-yellow-600">{{ number_format($stats['pending_approvals']) }}</div>
                            <div class="text-xs text-gray-500">Pending</div>
                        </div>
                        @if($stats['expiring_licenses'] > 0)
                        <div class="text-center min-w-[80px] md:min-w-0">
                            <div class="text-xl md:text-2xl font-bold text-red-600">{{ number_format($stats['expiring_licenses']) }}</div>
                ...
                <!-- Quick Filters -->
                <div class="flex flex-col sm:flex-row gap-4 pb-4 border-b border-gray-100">
                    <div class="flex items-center space-x-2">
                        <label class="text-sm text-gray-600">Period:</label>
                        <select wire:model.live="selectedPeriod" class="text-sm border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <div class="flex items-center space-x-2">
                        ...
                        <select wire:model.live="selectedInstitutionType" class="text-sm border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500">
